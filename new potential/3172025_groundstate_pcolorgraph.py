...
class SimulationConfig:
    ...
    def __init__(self):
        ...
        self.Nt = [200, 500, 1000, 1500, 2000, 2500, 3000]
...
def setup_grid(config):
    ...
    try:
        ...
        V_final = -5*(np.exp(-((X-4)**2) / (2*3)))*(np.exp(-((Y+3)**2) / (2*4))) - 5*(np.exp(-((X+5)**2) / (2*(5)))) * (np.exp(-((Y+5)**2) / (2*(5))))  - 4*(np.exp(-((X)**2) / (2*3)))*(np.exp(-((Y-5)**2) / (2*3))) + 0.01 * (X**2 + Y**2)
...
plt.title('Adiabatic Evolution: Overlap vs Transition Time (More Advanced Potential)', fontsize=14, pad=20)
...
plt.tight_layout()
plt.savefig('overlap_vs_time_advanced.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('ground_state_density_step3.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.axis('square')  # Make plot square since system is symmetric
plt.tight_layout()
plt.savefig('ground_state_density_psireal.png', dpi=300, bbox_inches='tight')
plt.show()
